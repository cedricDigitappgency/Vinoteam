...
@section('content')
				...
				<p style='margin:0;padding:0;font-family:"Helvetica Neue", "Helvetica", Helvetica, Arial, sans-serif;margin-bottom:10px;font-weight:normal;font-size:14px;line-height:1.6'>Bonjour {{ $buyer->firstname }} {{ $buyer->lastname }},<br />
					Le remboursement numéro {{ $order->id }} effectué le {{ date('d-m-Y', strtotime($order->created_at)) }} vers {{ $owner->firstname }} {{ $owner->lastname }} n'a pas été accepté par votre banque.<br />
					Afin de valider ce remboursement, nous vous invitons à consulter votre banque. <br />
